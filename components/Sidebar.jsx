...
const Sidebar = ({expand, setExpand}) => {
    ...
    <div className={`flex flex-col justify-between bg-[#212327] pt-7 transition-all z-50 max-md:absolute max-md:h-screen ${expand ? 'p-4 w-64' : 'md:w-20 w-0 max-md:overflow-hidden'}`}>
      <div>
        <div className={`flex ${expand ? 'flex-row gap-10' : 'flex-col items-center gap-8'}`}>
            <Image className={expand ? ' h-15 mt-2' : 'w-10'} src={expand ? assets.logo_icon2 : assets.logo_icon} alt=''/>
  ...
  className={`group mt-8 flex items-center justify-center cursor-pointer transition-all
  ${expand
    ? 'bg-gray-500 hover:opacity-90 rounded-3xl gap-2 p-2.5 w-max'
    : 'relative h-9 w-9 mx-auto hover:bg-gray-500/30 rounded-lg'
  }`}
>
  <Image
  ...
  {/* Tooltip (always exists) */}
  <div
    className={`absolute w-max -top-12 -right-12 transition
    bg-black text-white text-sm px-3 py-2 rounded-lg shadow-lg pointer-events-none
    ${expand ? 'opacity-0' : 'opacity-0 group-hover:opacity-100'}
    `}
  >
    New Chat
  ...
  {/* Text (always exists, visibility controlled) */}
  <p
  ...
  >
    New Chat
        ...
        <div className={`mt-8 text-white/45 text-sm ${expand ? 'block' : 'hidden'}`}>
      ...
      <div>
        <div className={`flex items-center cursor-pointer group relative ${expand ? 'gap-1 text-white/80 text-sm p-2.5 border border-gray-500 rounded-lg hover:bg-white/10 cursor-pointer' : 'h-10 w-10 mx-auto hover:bg-gray-500/30 rounded-lg'}`}>
